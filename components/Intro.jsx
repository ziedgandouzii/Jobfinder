...
export default function Intro() {
  ...
  const handleSearch = () => {
    const filteredJobs = jobData?.filter((job) => {
      ...
      // Check if the search term matches any of the fields (title, company name, or job category)
      const titleMatches = job?.title?.toUpperCase().includes(searchTerm);
      const companyMatches = job?.company?.toUpperCase().includes(searchTerm);
      const categoryMatches = job?.job_category?.toUpperCase().includes(searchTerm);

      // Return true if any of the fields match the search term
      return titleMatches || companyMatches || categoryMatches;
    });
    ...
    <>
      <div className='w-full  h-full flex items-center lg:justify-start py-24 justify-center flex-wrap  '>
        <div className='lg:w-3/6 w-full sm:p-2 h-full my-2 flex items-center justify-center px-4 md:items-start md:justify-start md:p-20 flex-col '>
          <h1 className='md:text-6xl text-2xl sm:text-2xl font-extrabold mb-4 text-black '>To Find <span className='text-indigo-600'>Right Jobs.</span> </h1>
          ...
          <div className='bg-white flex-col mb-6 w-full md:px-4   py-4 flex sm:flex-row items-center justify-center'>
            ...
            <input
              ...
              type="text"
              placeholder="Search Jobs By Name,Category or Company Name ..."
